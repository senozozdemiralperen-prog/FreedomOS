...
if page == "🏠 Genel Durum & Özet Paneli":
    ...
    if st.session_state.income_expense_history.empty or st.session_state.investment_history.empty:
        st.info("👋 FreedomOS'a Hoş Geldiniz! Lütfen önce veri girişlerini tamamlayın.")
    else:
        ...
        # Mark Tilbury Mentorluk Mesajı Alanı
        ...
        if tasarruf_orani >= 50:
            ...
            st.info(f"👍 **İyi Durumdasınız:** Tasarruf oranınız **%{tasarruf_orani:.1f}**. %1'lik kesimin altın kuralı olan minimum %20 barajını geçtiniz. Yatırımları büyütebilirsiniz.")
        else:
...
# --- SAYFA 2: GELİR / GİDER ---
    ...
    # Mevcut form yapısı korundu, Sheets kaydı stabilize edildi
...
elif page == "📈 Varlık & Yatırım Takibi" or page == "📈 Varlık & Portföy Rebalancing":
    st.header("📈 Akıllı Portföy Yönetimi & Rebalancing (Dengeleme)")
    
    # Stratejik Hedef Belirleme Alanı (Mark Tilbury Modeli)
    ...
    if not st.session_state.investment_history.empty:
        ...
        for index, row in rebal_df.iterrows():
            ...
            if fark_tl > 0:
                ...
                st.warning(f"✂️ **{row['Varlık Tipi']}** portföyünüzde hedefinizden fazla yer kaplıyor. **{abs(fark_tl):,.0f} TL** değerinde kâr satışı düşünülebilir.")
...
# --- SAYFA 4: ÖNGÖRÜ VE YAPAY ZEKA ---
elif page == "🔮 İleri Yönelik Öngörü & Yapay Zeka" or page == "🔮 %1 Kesim İleri Öngörü & Yapay Zeka":
    ...
    if len(st.session_state.income_expense_history) >= 2:
        ...
        for m in range(1, sim_months + 1):
            ...
            passive_gen = (current_portfolio * 0.04) / 12  # %4 Güvenli Çekim Kuralı
        ...
        # Kilometre Taşları Hesaplamaları
        st.markdown(f"""
        * **Aşama 1: Finansal Güvence Kapısı:** Acil durum fonunuz ve temel giderlerinizin koruma altında olması.
        * **Aşama 2: %4 Kuralı Entegrasyonu:** Yatırımlarınızın aylık getirisi şu an **{df_sim.iloc[-1]['Pasif Gelir']*0.1:,.0f} TL** pasif akış üretme kapasitesine sahip.
        """)
        
        if freedom_m != -1:
            st.balloons()
            st.success(f"🥳 **Tebrikler!** Mevcut finansal disiplininizle **{freedom_m // 12} Yıl {freedom_m % 12} Ay** sonra finansal olarak tamamen özgürsünüz.")
        else:
            st.error("⚠️ Mevcut girdiler ve enflasyon baskısı altında 30 yıl içinde hedefe ulaşılamıyor. Tasarrufları veya getiri oranını artırmalısınız.")
